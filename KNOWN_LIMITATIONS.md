...
- Memory-only changes cannot survive a page close or reload. When the persistent warning is visible, use Export Backup Now before leaving the app.
...
- For responsiveness, the Transactions page renders the first 300 matching records. All records remain stored, included in totals/exports, and searchable; narrowing the filters reveals older matches.
- A pre-import JSON backup is stored under a separate local key for rollback safety, but Settings does not currently provide a dedicated browser for older automatic pre-import backups.
